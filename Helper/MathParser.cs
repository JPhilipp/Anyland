...
namespace MathParserTK {
    ...
    // via https://www.codeproject.com/Tips/381509/Math-Parser-NET-Csharp -Philipp
    ...
    public class MathParser
    {
        ...
        private const string Tg = FunctionMarker + "tan";
        ...
        private const string Abs = FunctionMarker + "absolute";
        
        private const string Round = FunctionMarker + "round";
        private const string Random = FunctionMarker + "random";
        private const string RandomFloat = FunctionMarker + "randomfloat";
        private const string Mod = FunctionMarker + "mod";
        private const string Ceil = FunctionMarker + "ceil";
        private const string Floor = FunctionMarker + "floor";
        private const string Smaller = FunctionMarker + "smaller";
        private const string Larger = FunctionMarker + "larger";
        ...
        private string LexicalAnalysisInfixNotation(string expression, ref int pos)
        {
                ...
                || supportedConstants.ContainsKey(token.ToString()))
            {
                ...
                {
                    // throw new ArgumentException( "Unknown token " + token.ToString() ); -Philipp
                    return NumberMaker + "0";
                }
            ...
            else if (Char.IsDigit(token[0]) || token[0] == decimalSeparator)
            {
                ...
                else
                {
                    ...
                    // token.Clear(); // requires .Net 4.0, using below workaround to clear -Philipp
        ...
        private Stack<double> SyntaxAnalysisRPN(Stack<double> stack, string token)
        {
            ...
            else if (NumberOfArguments(token) == 1)
            {
                ...
                switch (token)
                {
                    ...
                    case Round: 
                        ...
                        break;
                    case Ceil: 
                        rst = Mathf.Ceil( (float)arg ); 
                        break;
                    case Floor: 
                        rst = Mathf.Floor( (float)arg ); 
                        break;
                    default:
            ...
            else // if ( NumberOfArguments(token) == 2 ) {
            {
                ...
                switch (token)
                {
                    ...
                    case Random:
                        rst = (float)UnityEngine.Random.Range( (int)arg1, (int)(arg2 + 1f) );
                        break;
                        
                    case RandomFloat:
                        rst = UnityEngine.Random.Range( (float)arg1, (float)arg2 );
                        break;

                    case Mod:
                        rst = (float)arg1 % (float)arg2;
                        break;
